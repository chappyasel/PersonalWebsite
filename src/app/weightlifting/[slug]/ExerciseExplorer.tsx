...
const METRICS: {
  ...
  { key: "oneRM", pickerTitle: "One-Rep Max", graphTitle: "1RM Equivalents", badgePrefix: "1RMe: ", unit: "lbs", setValue: (s) => s.oneRM ?? 0 },
  { key: "volume", pickerTitle: "Volume", graphTitle: "Total Volumes", badgePrefix: "Tot: ", unit: "lbs", setValue: (s) => s.volume ?? 0 },
  { key: "maxWeight", pickerTitle: "Max Weight", graphTitle: "Max Weights", badgePrefix: "Max: ", unit: "lbs", setValue: (s) => s.weight ?? 0 },
  { key: "reps", pickerTitle: "Total Reps", graphTitle: "Total Reps", badgePrefix: "Tot: ", unit: "reps", setValue: (s) => s.reps ?? 0 },
  { key: "maxReps", pickerTitle: "Max Reps", graphTitle: "Max Reps", badgePrefix: "Max: ", unit: "reps", setValue: (s) => s.reps ?? 0 },
];
...
function computeInstanceMetrics(instance: ExerciseInstance): InstanceWithMetrics {
  let oneRM = 0;
  let maxWeight = 0;
  let maxReps = 0;
  let volume = 0;
...
function podiumSubtitle(instance: InstanceWithMetrics, metric: MetricKey): string {
  switch (metric) {
    case "oneRM":
      return instance.bestSetDescription;
    case "maxWeight": {
      const set = instance.sets.find((s) => s.weight === instance.values.maxWeight);
...
/** BTSet.description for reps_weight chips: "8x225", no spaces */
function chipText(set: ExerciseInstance["sets"][number]): string {
  if (set.reps != null && set.weight != null) return `${set.reps}x${Math.round(set.weight)}`;
...
function GraphTooltip({
  ...
  // BTRoundMarker: pastel pill, darkened text; 1RM shows "8 x 225" above the value
  return (
    <div className="rounded-full bg-[var(--seg-bg)] px-2.5 py-1.5 text-center text-[10px] font-semibold leading-tight text-[var(--seg-tx)] dark:bg-[var(--seg-bg-d)] dark:text-[var(--seg-tx-d)]">
...
export function ExerciseExplorer({
  ...
  const spanCutoff = useMemo(() => {
    ...
    return new Date(Date.now() - span.days * 86400_000).toISOString().slice(0, 10);
  }, [queryType, timeSpan]);

  const spanFiltered = useMemo(
    () => (spanCutoff ? withMetrics.filter((i) => i.date >= spanCutoff) : withMetrics),
  ...
  const graphData = useMemo(() => {
    ...
    if (queryType === "average") {
      ...
      if (n < 36) {
        let sum =
          chronological[0]!.values[metricKey] + chronological[1]!.values[metricKey];
        for (let i = 2; i < n; i++) {
        ...
        for (let i = n % num; i < n; i += num) {
          let sum = 0;
          for (let j = i; j < i + num; j++) sum += chronological[j]!.values[metricKey];
          rows.push({
            _i: rows.length,
  ...
  const summaryStats = useMemo(() => {
    ...
    return [
      { label: "Total Volume", value: shortenValue(Math.round(totalVolume)), unit: "lbs" },
      { label: "Total Sets", value: totalSets.toLocaleString(), unit: "" },
      { label: "Average 1RMe", value: shortenValue(Math.round(averageOneRM)), unit: "lbs" },
    ];
  }, [withMetrics]);
                ...
                <div key={rankIndex} className="flex flex-col justify-end">
                  ...
                  >
                    {instance ? (
                      <>
                        <p className="text-center font-bold leading-tight text-neutral-700 dark:text-neutral-200">
                          <span className="text-base">
                            {shortenValue(Math.round(instance.values[metricKey]))}
                          </span>{" "}
                      ...
                      <p className="text-base font-bold text-neutral-400">-</p>
                    )}
                  </div>
          ...
          <ChartContainer config={config} className="aspect-auto h-[240px] w-full">
            ...
            >
              ...
              <XAxis
                ...
                tickLine={false}
                axisLine={{ className: "stroke-neutral-300 dark:stroke-neutral-600" }}
                tickMargin={10}
                angle={-20}
              ...
              />
              <ChartTooltip
                cursor={{ stroke: "var(--seg-bg)", strokeWidth: 2 }}
                content={<GraphTooltip metric={metric} queryType={queryType} />}
              />
              <Area
                type="linear"
                dataKey="value"
                    ...
                    <ChartContainer config={{}} className="aspect-auto h-[200px] w-full">
                      ...
                      >
                        ...
                        <YAxis
                          ...
                          width={40}
                        />
                        {Array.from({ length: setProgressions.lineCount }, (_, i) => (
                          <Line
                            key={i}
                            type="linear"
                            dataKey={`line${i}`}
                            stroke={color}
                            strokeOpacity={i === setProgressions.lineCount - 1 ? 1 : 0.25}
                            strokeWidth={2.5}
                            dot={false}
                            isAnimationActive={false}
                          />
                ...
                {workoutSplit.length > 0 && (
                  <div className="rounded-2xl border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-800">
                    ...
                    <div className="flex flex-col items-center gap-4 md:flex-row">
                      <ChartContainer config={{}} className="aspect-square h-[185px]">
                        <PieChart>
                          <Pie
                            data={workoutSplit}
                            dataKey="count"
                            ...
                            label={(props: {
                              ...
                              const r =
                                ((props.innerRadius ?? 0) + (props.outerRadius ?? 0)) / 2;
                          ...
                          >
                            {workoutSplit.map((slice) => (
                              <Cell key={slice.name} fill={color} opacity={0.4} />
                            ))}
                          </Pie>
                        </PieChart>
                      </ChartContainer>
                      <ul className="w-full flex-1 space-y-1 text-sm">
                        {workoutSplit.map((slice) => (
                          <li key={slice.name} className="flex items-baseline gap-2">
                            <span
      ...
      <section className="space-y-2.5">
        ...
        <AnimatePresence initial={false}>
          {queryType === "top" && (
            ...
            >
              <SegmentedControl
                options={TIME_SPANS.map((s) => ({ key: s.key, label: s.label }))}
                value={timeSpan}
        ...
        {listed.length === 0 ? (
          ...
          <ul className="divide-y divide-neutral-100 dark:divide-neutral-700/60">
            {listed.map((instance, i) => (
              <li key={`${instance.ts}-${i}`} className="px-3.5 py-2">
