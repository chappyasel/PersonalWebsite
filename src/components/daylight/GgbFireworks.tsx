 ...
 * the additive is pulled back.
 *
 * Coordinates map through the SKYLINE STRIP's live rect each frame, not the
 * container's: on phones the strip is an overgrown panning element, and the
 * pan is a camera move — the shells live in that sky, so they ride it, and
 * the tap target rides it too (its own copy of the pan in daylight.css).
 *
 * The scene's composer softness is two blit passes from one half-resolution
 * offscreen render: a wide-blurred bloom canvas under a barely-blurred core
 * canvas. Both ride at z -1 inside the sky's stacking context, so every
 * shell bursts BEHIND the bridge, the buildings, and the hills — depth is
 * paint order, the shader block's own opening lesson.
 */
...
export default function GgbFireworks() {
  ...
  const draw = useCallback(() => {
    ...
    if (launches.current.length > 0) {
      const dark = document.documentElement.classList.contains("dark");
      // The strip's own scale and pan offset: aspect-true, so one number
      // serves both axes, and the strip's bottom edge is e = 0.
      const ppr = sRect.width / AZ_SPAN;
      const X = (az: number) => (az - AZ_LEFT) * ppr + (sRect.left - cRect.left);
      const Y = (e: number) => sRect.bottom - cRect.top - e * ppr;
      const time = now / 1000;
      ...
      for (const launch of launches.current) {
        ...
        for (let si = 0; si < 7; si++) {
          ...
          // Daytime: the additive is pulled back and the smoke does the
          // work — a puff DARKER than the sky behind it.
          if (!dark) {
            const win =
          ...
          // The stars. A ring is drawn with more, thinner rays because it
          // has to close; everyone else gets a per-ray velocity, which is
          // what gives a peony its depth.
          ...
          // White-hot at the burst, into the shell's own colour, cooling to
          // a dim ember as it falls.
          ...
          for (let bi = 0; bi < bins; bi++) {
            ...
            const twinkle =
              1 -
              twd +
              twd * (0.5 + 0.5 * Math.sin(time * 41 + bi * 2.7 + fs));
    ...
    // soft core (their blurs live in daylight.css).
