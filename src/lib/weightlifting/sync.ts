...
export async function syncWeightlifting(
  ...
  try {
    ...
    // 3. Full replace — truncate and reinsert
    console.log("Syncing weightlifting data...");

    // Delete in dependency order (sets → exercises → workouts → types)
    // eslint-disable-next-line drizzle/enforce-delete-with-where
    await db.delete(wlSets);
    // eslint-disable-next-line drizzle/enforce-delete-with-where
    await db.delete(wlExercises);
    // eslint-disable-next-line drizzle/enforce-delete-with-where
    await db.delete(wlWorkouts);
    // eslint-disable-next-line drizzle/enforce-delete-with-where
    await db.delete(wlExerciseTypes);
    ...
    if (uniqueTypes.length > 0) {
      await db.insert(wlExerciseTypes).values(
        uniqueTypes.map((t) => ({
          name: t.name,
          category: t.category,
          style: t.style,
    ...
    for (let i = 0; i < setRows.length; i += BATCH_SIZE) {
      ...
      await db.insert(wlSets).values(batch);
    }
