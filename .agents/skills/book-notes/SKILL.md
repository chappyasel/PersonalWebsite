...
## Hybrid cover emoji pipeline

A separate Vercel cron, `/api/cron/sync-book-emojis`, runs at 09:20 UTC with a 300-second limit. It requires `CRON_SECRET` authorization and does nothing unless `BOOK_COVER_EMOJIS_ENABLED=true`. Deployment also needs `BOOK_COVER_EMOJIS_WORKSPACE_ID`, the verified target workspace UUID, plus the existing `DATABASE_URL`, `AWS_BUCKET_NAME`, `AWS_REGION`, and AWS credentials. Keep the enabled flag off until the coordinator approves the pilot.

The cloud process reads a consistent, read-only snapshot of the existing mirror. It reuses the cover generator's grouping, bounded download, and transparent 512-pixel PNG renderer. Rereads share one work item with every exact Notion page ID. Assets, immutable revisions, conditional head pointers, source fingerprints, and the traversal cursor live under the existing S3 bucket's `book-cover-emojis/` prefix. It never writes to Postgres or Notion.

Each invocation attempts at most 20 cover downloads and stops starting work near its time limit. Successful jobs survive interrupted runs. The cursor advances past failures so later books still receive work. Unchanged source fingerprints skip downloads; covers at stable URLs are checked again after seven days. Changed page mappings advance the job revision, while changed artwork also advances the custom emoji name. A stored name that no longer matches the shared naming policy also receives a new immutable job revision, reported as `nameChanged`, without advancing the artwork revision. The live pilot established a 49-character maximum. Cached heads are checked against the current naming function on every visit, so invalid old names bypass the seven-day cover cache. Missing covers and failed downloads remain failures, with no placeholder job.

The local headless worker is a separate step. It registers reusable workspace custom emojis, verifies them through the Notion API, and applies their IDs to the listed pages. Cloud code imports no browser or cookie modules. Running the cron queues work only. It does not activate the local scheduler or authorize Notion writes.

The mirror has per-book sync timestamps but no durable whole-run success marker. This cron therefore reads the last available consistent mirror, which can include completed rows from a partially failed books sync. It does not invoke or change `/api/cron/sync-books`. Blank and undated Notion pages remain outside this pipeline until they enter the mirror.
