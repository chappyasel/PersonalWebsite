...
- [x] 168. Replace the empty lower-frame void with a continuous rolling meadow that extends from the skyline past the shelves to the camera in both themes. Adapt FluffyGrass's alpha-card principles into an original, bounded instanced field with generated tuft alpha, darker roots, varied tips, slow wind, flower points, theme-aware green, fog integration, and a terrain-only degraded fallback.
- [x] 169. Reduce the meadow's visible grass-blade height to one third of the first pass so the shelf feet sit in soft lawn rather than knee-high reeds.
- [x] 170. Replace screen-position tilt blur with real depth-of-field on the desktop composer: shelf/prop depth remains sharp while near meadow and distant hills/skyline receive restrained bokeh. Preserve `?notiltshift` and add `?nodof` comparison escapes; touch and degraded paths keep their zero-composer performance floor.
- [x] 171. Retune the primary light-mode vault to the approved pastoral reference: a distinctly blue upper sky grading through pale warm tan/yellow at the horizon, retaining the existing white wispy cloud forms, sun/skyline occlusion, dark mode, and seated Washington treatment.
- [x] 172. Remove every discoverable meadow boundary systemically: give terrain full-traverse frustum overscan, extend it well past complete fog convergence, shape the distant contact as rolling hills instead of a rectangular edge, fade vegetation fully before its distribution ends, and derive the landscape fog uniforms from the shared theme palette.
- [x] 173. Table the meadow experiment behind a single disabled lazy boundary: preserve its implementation, but request, evaluate, render, and animate none of it until explicitly re-enabled.
- [x] 174. Double the visible light-mode blue-cap contribution and deepen its authored blue while leaving the cream/tan horizon, clouds, dark palette, and seated Washington sky unchanged.
- [x] 175. Strengthen every World-mode text tier over the variable scene: promote Book Notes' near-black placard ink to the shared light foreground, move muted light copy closer to black, lift dark foreground/muted copy toward white, and remove the wordmark's extra opacity dilution while leaving dedicated reading routes unchanged.
- [x] 176. Remove the reverse-theme edge treatment from the complete persistent chrome system: desktop and mobile section glyphs/labels, the active underline, and the mobile wordmark render as clean single-colour foreground marks while retaining explicit keyboard focus rings.
- [x] 177. Increase desktop section-rail icons and labels by roughly twenty percent while reducing its row step by twenty percent; make Up/Down navigate the same bounded previous/next unit path as Left/Right, including cancellation of the initial About sync.
- [x] 178. Relax the enlarged desktop rail to a midpoint 2.25rem row step, and move the About couch twenty percent farther left plus slightly deeper while deriving its model, contact pool, and seated eye/target from one shared placement contract.
