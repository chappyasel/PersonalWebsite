...
- [x] 105. Add a subtle black-and-white silhouette of Chappy's childhood home on the left side of the seated DC vista, preserving its yellow front door and using `/Users/chappyasel/Desktop/640x480.jpg` as the visual reference.
...
- [x] 108. Make the golf club draggable while preserving a distinct tap interaction that performs a full swing and launches a nearby ball a convincingly long distance.
...
- [x] 113. Eliminate the brief full-sheet flash when About or Featured Talks transitions into the fully collapsed mobile pill.
...
- [x] 115. Lower the Talks microphone so its body makes believable shelf contact.
...
- [x] 118. Separate the slightly intersecting Projects trophy/photo arrangement while retaining the current hierarchy.
...
- [ ] 128. Make every visible Book Notes book independently tactile: all eight data-backed covers open their exact details on tap and carry without opening on drag; decorative spines and stacks respond without fake destinations; hover motion never intersects a neighbor; and the shimmering secret spine remains discoverable without stealing plant, scrolling, or cover interactions.
...
- [ ] 131. Make the inter-unit floor lamp read as an almost blinding practical source, with a much brighter aperture and believable bloom/spill that remains controlled in light mode and does not clip nearby photography or UI.
- [ ] 132. Raise the foreground illumination of shelves and props across dark mode so objects remain readable without washing out the navy sky, moon, skyline silhouettes, or warm practical-light contrast.
- [x] 133. Rebuild About's three-current-book arrangement against `/Users/chappyasel/Desktop/three-colored-books-icon-cartoon-style-white-background_96318-13055.avif`: the two older volumes form a compact, nearly horizontal overlapping base while the newest plants its lower board directly on the shelf to their left/rear, leans twenty to thirty degrees from vertical toward their rear-left face, and is slightly occluded by that horizontal stack at its base. Give the planted edge a small shelf-level contact shadow. Give each volume a cream page block plus cloth boards and spine tinted from a stable server-side perimeter sample of its own cover (with a deterministic fallback); a grabbed book turns its cover toward the viewer and settles to its exact authored contact pose, while the small plant remains grounded on the lower shelf between the stack and photo with positive clearance.
...
- Compare About's reading stack with the supplied three-colored-books reference in desktop light, desktop dark, and 390-pixel mobile captures: reject separated ramps, gaps, intersections, or shadow-float; record each cover-derived edge color and exercise both tap and drag on all three books, proving a carry opens no detail before exact settle.
...
The final World-mode registry contains 73 draggable instances:

- About (13): Apple, AI Collective mark, dumbbell, four loose photos, three plants, and three current-reading books.
- Book Notes (8): all featured face-out covers. A tap opens the exact book note; a carry does not.
- Weightlifting (19): basketball, three branded cans, two dumbbells, four golf balls, golf club, kettlebell, two photos, three books, plant, and protein tub. A tap on the club still performs the authored full swing and ball launch.
...
- Systems (10): six personal-photo frames, three books, and sansevieria.

Intentional exclusions are shelves and legs; the seated chair/couch; fixed lamp rigs; the main portrait and mounted Training board/pins; the display-stand globe; clocks/cases, the routine board, Macintosh, and other dedicated mechanisms; packed background book rows/bookends; and the loaded inter-unit barbell and spare bumpers. These are structural, wired, mounted, heavy/racked, or already own a more specific interaction, so wrapping them would steal or nest pointer behavior rather than make the world more consistent.
