...
4. Run `yarn youtube:evaluate-scores`.
...
```sh
yarn youtube:backfill-metadata --force
```

Channel avatars, refreshed when missing or older than 30 days:

```sh
yarn youtube:backfill-channels --execute
```
