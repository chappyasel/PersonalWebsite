...
`check:room-artwork`, already in `verify:artifacts`, verifies the frozen quality input checksums, corrected alpha provenance, crops, owner inventory and image dimensions before checking generated outputs. It also compares every final owner with its frozen fresh mask at the current desktop and phone display sizes. The conservative Manhattan residual must stay within 3 CSS pixels. Run `pnpm check:room-artwork-quality` to repeat that geometry check alone. `check-repackage.mjs` compares every active input SVG/detail, immutable camera contract, manifest, frozen specification and receipt before and after another full packaging pass.
...
```sh
for unit in books projects systems weightlifting musings talks
do
  for variant in light-desktop dark-desktop light-phone dark-phone
  do
    node scripts/room-artwork-quality/capture.mjs "$unit" "$variant" --base http://localhost:3338 --out /tmp/new-room-captures
    node scripts/room-artwork-quality/package.mjs "$unit" "$variant" --input /tmp/new-room-captures
  done
done
