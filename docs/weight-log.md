...
`/weight-log` is a read-only chart behind the existing private-page password and signed access cookie. It opens on the full history with only the seven-day trend visible. The Layers control adds the other series, and About this chart holds the calculation notes. Users can select dates or a phase, and inspect exact weigh-ins in a year calendar. DEXA values also have a table.
