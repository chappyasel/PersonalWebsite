...
`/weight-log` is a read-only chart behind the existing private-page password and signed access cookie. It opens on the full history with only the seven-day trend visible. The Layers control adds the other series, and About this chart holds the calculation notes. Dragging across the plot with a mouse or pen zooms to the selected dates in either direction. Reset zoom restores the range before the first drag; manual date or phase changes establish a new range. Escape cancels a drag, and touch scrolling stays available. A horizontal hover guide snaps to the nearest visible series at the hovered date and labels its value in pounds or percent. Mouse clicks have no focus outline; keyboard focus remains visible. Users can select dates or a phase, and inspect exact weigh-ins in a year calendar. DEXA values also have a table.
...
The default-off `Early body fat · exploratory` layer extends estimates back through available weight history. It uses phase colors with a fainter dotted line and shaded sensitivity band, and marks the first DEXA. It can be shown with or without the later body-fat layer. Hovering shows the estimate and full range. The percentage axis retains its 4% floor, clipping any lower band values visually without changing them. Hidden early estimates do not expand the axis. Model fitting precedes date filtering and zooming.

`src/lib/weight-log/historical-body-fat.ts` fits `FFM = first scan FFM + k * change in weight`. Anchored least squares constrains `k` to zero through one and adds 25 square pounds to its denominator, a 5 lb regularization scale that shrinks unsupported slopes toward constant fat-free mass. At least four usable scans and a weight difference of at least 2 lb are required. The scale-to-scan offset at the first scan is held constant for earlier dates. Impossible compositions and missing weight estimates remain gaps. This model assumes equal body composition at equal weights; it cannot identify earlier muscle gain at a stable bodyweight.

Backward validation hides each early scan and all scans before it, then predicts that scan from at least three later scans. The UI reports the resulting average absolute percentage-point error, count, and longest tested horizon. These checks do not establish accuracy for earlier training years outside that horizon.

The sensitivity band's half-width in fat-free mass is `max(backward RMSE, 0.02 * anchor weight) * sqrt(1 + days before anchor / median validation horizon) + abs(weight change) * maximum leave-one-scan-out slope deviation`. Percentage endpoints are bounded to zero through 100. The minimum width and time-widening rule are explicit heuristic choices. The band is not a confidence interval, a calibrated prediction interval, or a claim about the probability of containing historical body fat. It includes uncertainty at the first scan rather than collapsing to zero.

Strength history was evaluated as an additional predictor using comparable lift variations, low-repetition sets, local time windows, and backward-held-out scans. It did not materially improve prediction in that comparison, so the shipped model has no strength-to-muscle conversion or new database dependency. In trained lifters, strength and lean-mass changes can have weak associations over a training block, as shown in [Bagheri et al.](https://pubmed.ncbi.nlm.nih.gov/39206316/). Hydration and glycogen also affect DEXA lean-mass measurements, as shown in [Toomey et al.](https://pubmed.ncbi.nlm.nih.gov/28204901/). These studies motivate caution; they do not validate the historical model or its band. Private evaluation data and fitted personal values remain outside Git.
