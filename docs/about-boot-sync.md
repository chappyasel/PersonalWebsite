...
The loading vignette is the About shelf at its canonical rest pose. It is not a
separately authored illustration. This file records which module owns each part
of that pose and how a change reaches the SVG.
...
| TJ medallion          | `TJ_MEDALLION_SOLIDS` and `tj-medallion.jpg`                               | `TJ_MEDALLION_POSE`                                        | Generated posed outline with the real artwork on its projected face               | Geometry-spec digest, artwork URL, and pose-tie tests                             |
| Apple Vision Pro      | Optimized `vision-pro.glb` with distinct band, enclosure, and glass meshes | Shared scale, two-point contact seat, and rotation         | Smoothed generated layered traces; glass mask expanded within the front enclosure | Asset and pose digests; profile, glass coverage, and front/rear contact tests     |
...
| Shelf supports        | `SHELF_GEOMETRY.support` boxes                                             | Shared X inset, Z depth, lower shelf, and ground planes    | Depth-projected uprights and feet; lower cleats omitted                           | Markup tests independently project each visible box corner                        |
