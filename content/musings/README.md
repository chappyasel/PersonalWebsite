...
Cards and article pages estimate reading time at 230 words per minute, rounded up. Local cards use the full `searchText`, never the preview description. External entries can provide `readingWordCount`; the sync preserves it in the post list. Trust's 6,489-word count comes from the essay body in the AI Collective website's `src/app/(main)/trust/page.tsx`, checked September 13, 2026. Update that count when the external essay changes.
